...
const ViewToggle = ({ viewMode, setViewMode, className }) => {
  const handleViewChange = (mode) => {
    console.log(
      `[ViewToggle] Setting view mode to: ${mode}`
    );
    setViewMode(mode);
  };

  return (
    <div className={cn("flex justify-center mb-4 gap-2", className)}> {/* Allow passing classes */} 
      <Button
      ...
      <Button
        ...
        onClick={() => handleViewChange("board")}
         className={cn(
          "transition-all duration-200",
          viewMode === 'board' ? 'bg-primary/10 text-primary' : 'text-muted-foreground'
        )}
      >
