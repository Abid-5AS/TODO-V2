...
export const getAIProviderStatus = async () => {
  try {
    ...
    // Expecting { success: boolean, data: { useLocalAI: boolean, status: 'connected' | 'disconnected' | 'unknown' } }
...
export const toggleAIProvider = async (useLocalAI) => {
  try {
    const response = await axiosInstance.post('/api/ai/toggle-provider', { useLocalAI });
     // Expecting { success: boolean, data: { useLocalAI: boolean, status: 'connected' | 'disconnected' | 'unknown' }, message?: string }
    return response.data;
  } catch (error) {
