  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-white p-2 shadow-md rounded-md border">
        <p className="font-semibold">{data.priority}: {data.count} tasks</p>
        <p className="text-sm opacity-80">{Math.round((data.count / data.total) * 100)}% of all tasks</p>
      </div>
    );
  }
  return null;
...
const TaskPriorityChart = ({ data = [], loading = false, index = 0 }) => {
